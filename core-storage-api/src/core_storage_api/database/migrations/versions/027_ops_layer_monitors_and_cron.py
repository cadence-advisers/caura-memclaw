...
One statement per ``op.execute`` — the migration driver (asyncpg) rejects
...
# Each entry is a SINGLE SQL statement (asyncpg: one command per execute).
_UPGRADE_STATEMENTS: tuple[str, ...] = (
    ...
    "CREATE SCHEMA IF NOT EXISTS cora_ops;",
    # ── tenant-isolation: tables, state, function, view ──
    """
    ...
    """,
    # ── enrichment-health: table, helper fn, check fn, view ──
    """
    ...
    """,
    # ── pg_cron scheduling — guarded + exception-wrapped (single DO statement) ──
    """
    DO $$
    BEGIN
...
_DOWNGRADE_STATEMENTS: tuple[str, ...] = (
    ...
    END $$;
    """,
    "DROP VIEW IF EXISTS cora_ops.v_enrichment_health;",
    "DROP VIEW IF EXISTS cora_ops.v_tenant_isolation_recent;",
    "DROP FUNCTION IF EXISTS cora_ops.check_enrichment_health();",
    "DROP FUNCTION IF EXISTS cora_ops._eh_raise(text, int, int, text);",
    "DROP FUNCTION IF EXISTS cora_ops.check_tenant_isolation();",
    "DROP TABLE IF EXISTS cora_ops.enrichment_health_alerts;",
    "DROP TABLE IF EXISTS cora_ops.tenant_isolation_alerts;",
    "DROP TABLE IF EXISTS cora_ops.tenant_isolation_monitor_state;",
    "DROP SCHEMA IF EXISTS cora_ops;",
    "DROP FUNCTION IF EXISTS public.infer_relations_cooccurrence();",
)
